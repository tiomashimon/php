...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['title']) && isset($_POST['summary']) && isset($_POST['text']) && isset($_POST['date']) && isset($_POST['views'])) {
        ...
        $fileData = json_encode($news);
        file_put_contents('news.json', $fileData);
    }
}

// Завантаження масиву об'єктів з файлу при запуску скрипта
if (file_exists('news.json')) {
    $fileData = file_get_contents('news.json');
    $loadedNews = json_decode($fileData, true);
    if ($loadedNews) {
        $news = $loadedNews;
    }
}

// Фільтрація новин (ваша поточна логіка залишається незмінною)
